...
import { createStrategy, createIdentityStrategy, createResponseHandler, createIdentityResponseHandler, TranslatedMatchingResponseBody, TranslatedIdentityResponseBody } from 'passport-verify'
...
function configureApp () {
  ...
  return { app, loadRequestId, saveRequestId, renderErrorPage }

}

export function createIdentityApp (verifyServiceProviderHost: string, entityId?: string) {
  const appConfig = configureApp()
  const app = appConfig.app
  const loadRequestId = appConfig.loadRequestId
  const saveRequestId = appConfig.saveRequestId
  const renderErrorPage = appConfig.renderErrorPage

  passport.use(createIdentityStrategy(
  ...
  app.post('/verify/response', (req, res, next) => {
    const authMiddleware = passport.authenticate('verify', createIdentityResponseHandler({
      onIdentityVerified: (identity) => {
        return res.render('view-identity-attributes.njk', { identity: identity })
      },
      onAuthnFailed: () => {
...
export function createMatchingApp (verifyServiceProviderHost: string, db: DatabaseWrapper, entityId?: string) {
  const appConfig = configureApp()
  const app = appConfig.app
  const loadRequestId = appConfig.loadRequestId
  const saveRequestId = appConfig.saveRequestId
  ...
  async function createUser (responseBody: TranslatedMatchingResponseBody) {
    let user = await db.createUser(responseBody)

    return Object.assign({ levelOfAssurance: responseBody.levelOfAssurance }, user)
  }

  async function verifyUser (responseBody: TranslatedMatchingResponseBody) {
