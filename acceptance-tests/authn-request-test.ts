...
describe('Clicking "Start"', function () {
  ...
  before(() => {
    const postData = {
      ...
      assertionConsumerServiceUrl: `http://localhost:${server.address().port}/verify/response`,
  ...
  it('Should handle a basic success response from the compliance tool', async () => {
    await browser.visit(`http://localhost:${server.address().port}`)
    await browser.clickLink('Start')
    browser.assert.status(200)
    browser.assert.url(`${COMPLIANCE_TOOL_HOST}/SAML2/SSO`)
    ...
    await browser.visit(responseGeneratorResponseJson.testCases[0].executeUri)
